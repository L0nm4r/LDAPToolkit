...
func (s ACE) String() string {
	...
	if sid.Resolve() == sid.String() {
		return fmt.Sprintf("SID: %s\n%s", sid.String(), sb.String())
	} else {
		return fmt.Sprintf("SID: %s - %s\n%s", sid.String(), sid.Resolve(), sb.String())
	}
}
...
func (header ACEHeader) FlagsString() string {
	var readableFlags []string
	flags, _ := bamflags.ParseInt(int64(header.Flags))
...
func (s BasicAce) GetRawObjectType() GUID {
	return GUID{}
}

func (s BasicAce) GetRawInheritedObjectType() GUID {
	return GUID{}
}
...
func (s AdvancedAce) GetRawObjectType() GUID {
	return s.ObjectType
}

func (s AdvancedAce) GetRawInheritedObjectType() GUID {
	return s.InheritedObjectType
}
...
type ObjectAce interface {
	...
	GetRawObjectType() GUID
	GetRawInheritedObjectType() GUID
}
